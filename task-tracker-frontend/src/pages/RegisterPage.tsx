  ...
  return (
    <div className="mx-auto max-w-md">
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-slate-400">Создайте аккаунт, чтобы вести список задач.</p>
      </div>
      ...
      <form
        ...
        onSubmit={async (e) => {
          ...
          try {
            ...
            nav(ROUTES.tasks, { replace: true })
          } catch (e: any) {
            setError(e?.response?.data?.error?.message ?? e?.message ?? 'Не удалось зарегистрироваться')
          } finally {
            setLoading(false)
          }
        }}
      >
        <label className="block">
          <div className="text-sm text-slate-300">Имя (необязательно)</div>
          <input
        ...
        <div className="text-sm text-slate-400">
          Уже есть аккаунт?{' '}
          <Link className="text-slate-200 underline decoration-slate-700 hover:decoration-slate-400" to={ROUTES.login}>
            Войти
          </Link>
        </div>
      </form>
